...
type Props = {
  themes: Template[];
  /** true if user may use premium themes (subscription, dev bypass, or paid credits). */
...
export function ThemeGrid({ themes, canAccessPremium, freeQuotaExhausted = false, onSelect }: Props) {
  return (
    <FlatList
      data={themes}
      keyExtractor={(item) => item.slug}
      renderItem={({ item }) => {
        const premiumLocked = item.isPremium && !canAccessPremium;
